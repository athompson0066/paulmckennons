...
export default function PainPointSection() {
  ...
  const options = [
    { title: "I want to Sell", subtitle: "Top dollar guaranteed", icon: <Home className="w-5 h-5" />, hoverClass: "hover:bg-brand-red hover:text-white", agent: "Elias" },
    { title: "I want to Buy", subtitle: "Dream GTA neighborhoods", icon: <Search className="w-5 h-5" />, hoverClass: "hover:bg-brand-blue hover:text-white", agent: "Maya" },
    { title: "Vetted Contractor", subtitle: "Paul's private network", icon: <Hammer className="w-5 h-5" />, hoverClass: "hover:bg-slate-800 hover:text-white", agent: "Vince" },
  ];
        ...
        <div className="md:w-2/3 grid grid-cols-1 md:grid-cols-3 gap-4">
            {options.map((option, i) => (
                <button 
                  key={i} 
                  onMouseEnter={() => {
                    setActiveAgent(option.agent);
                    setChatOpen(true);
                  }}
                  className={`flex flex-col items-center justify-center p-4 rounded-xl border border-slate-100 bg-slate-50 transition-all group ${option.hoverClass}`}>
